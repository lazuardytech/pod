...
const FE_ACTIVE_TIMEOUT_MS = 60000;const FE_ACTIVE_TICK_MS = 1000;
...
function ProviderNode({ data }: any) {
  ...
  const borderColor = error ? "#ef4444" : active ? "#22c55e" : "var(--color-border)";  const dotColor = error ? "#ef4444" : "#22c55e";  const textColor = error ? "#ef4444" : active ? "#22c55e" : "var(--color-text)";
  return (
    <div
...
function buildLayout(providers: any, activeSet: any, lastSet: any, errorSet: any) {
  const nodeW = 180;  const nodeH = 30;  const routerW = 120;  const routerH = 44;  const nodeGap = 24;
  const count = providers.length;
  // Compute rx so arc spacing between nodes >= nodeW + nodeGap
  const minRx = ((nodeW + nodeGap) * count) / (2 * Math.PI);  const rx = Math.max(320, minRx);  const ry = Math.max(200, rx * 0.55); // ellipse ratio ~0.55  if (count === 0) {
    return {
      nodes: [
        {
          id: "router",
  ...
  const edgeStyle = (active: any, last: any, error: any, _color: any) => {    if (error) return { stroke: "#ef4444", strokeWidth: 2.5, opacity: 0.9 };
  ...
  providers.forEach((p: any, i: any) => {
    const config = getProviderConfig(p.provider);    const active = activeSet.has(p.provider?.toLowerCase());    const last = !active && lastSet.has(p.provider?.toLowerCase());    const error = !active && errorSet.has(p.provider?.toLowerCase());    const nodeId = `provider-${p.provider}`;    const data = {      label: (config.name !== p.provider ? config.name : null) || p.name || p.provider,
    ...
    const angle = -Math.PI / 2 + (2 * Math.PI * i) / count;    const cx = rx * Math.cos(angle);    const cy = ry * Math.sin(angle);
...
function loadViewport() {
  try {
    const raw = sessionStorage.getItem(VIEWPORT_SESSION_KEY);    if (raw) return JSON.parse(raw);
  } catch {}
  return null;
}
...
export default function ProviderTopology({
  ...
  const lastKey = lastProvider?.toLowerCase() || "";  const errorKey = errorProvider?.toLowerCase() || "";
  ...
  useEffect(() => {
    const seen = firstSeenRef.current;    const now = Date.now();    for (const p of rawActiveSet) {
      const pk = p as string;      if (!seen[pk]) seen[pk] = now;
    }
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setTick((t: any) => t + 1), FE_ACTIVE_TICK_MS);    return () => clearInterval(id);
  }, [rawActiveSet]);

  /* eslint-disable react-hooks/exhaustive-deps */
  const activeSet: any = useMemo(() => {
    const now = Date.now();    const filtered = new Set<any>();    for (const p of rawActiveSet) {
      const pk = p as string;      const ts = firstSeenRef.current[pk];      if (!ts || now - ts < FE_ACTIVE_TIMEOUT_MS) filtered.add(pk);
    }
    return filtered;
  }, [rawActiveSet, tick]);
  ...
  const fitOpts = { padding: 0.2, duration: 200 };  const savedViewport: any = useRef(loadViewport());
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {      if (rfInstance.current && !savedViewport.current) rfInstance.current.fitView(fitOpts);
    });
    ro.observe(el);
  ...
  useEffect(() => {
    if (rfInstance.current && !savedViewport.current) {
      const id = setTimeout(() => rfInstance.current.fitView(fitOpts), 50);      return () => clearTimeout(id);
    }
    return undefined;
  }, [nodes.length]);
