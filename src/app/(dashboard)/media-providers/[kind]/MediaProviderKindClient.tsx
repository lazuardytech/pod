...
function getEffectiveStatus(conn: DashboardConnection) {
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          <div
            ...
            style={{
              backgroundColor: `${(provider.color?.length ?? 0) > 7 ? provider.color : (provider.color ?? "#888") + "15"}`,
            }}
          >
            <ProviderIcon
...
export default function MediaProviderKindPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/providers", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: { connections?: DashboardConnection[] }) => setConnections(d.connections || []))
      .catch(() => {});
    if (isEmbedding) {
      fetch("/api/provider-nodes", { cache: "no-store" })
        .then((r) => r.json())
        .then((d: { nodes?: CustomNode[] }) =>
