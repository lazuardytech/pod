...
export async function getProviderCredentials(
  ...
  try {
    ...
    const availableConnections = connections.filter((c): boolean => {
    ...
    if (availableConnections.length === 0) {
      const expiries = connections
        .map((c): string | null => getConnectionLockUntil(c) || getEarliestModelLockUntil(c))
    ...
    if (preferredConnectionId) {
      connection = availableConnections.find((c): boolean => c.id === preferredConnectionId);
      if (connection) {
        log.info(
          "AUTH",
    ...
    if (connection) {
      ...
      let current = state
        ? availableConnections.find((c): boolean => c.id === state?.lastConnectionId)
        : null;
      if (!current) {
        const byRecency = [...availableConnections].sort((a, b): number => {
      ...
      if (current && state!.consecutiveCount < stickyLimit) {
        ...
        const sortedByOldest = [...availableConnections].sort((a, b): number => {
          if (!a.lastUsedAt && !b.lastUsedAt) return (a.priority || 999) - (b.priority || 999);
          if (!a.lastUsedAt) return -1;
          if (!b.lastUsedAt) return 1;
...
export async function markAccountUnavailable(
  ...
  const conn = (connections as AnyConnection[]).find((c): boolean => c.id === connectionId);
...
export async function clearAccountError(
  ...
  const now = Date.now();
  const allLockKeys = Object.keys(conn).filter((k): boolean => k.startsWith("modelLock_"));
  const allLockCountKeys = Object.keys(conn).filter((k): boolean =>
  ...
  const keysToClear = allLockKeys.filter((k): boolean => {
  ...
  if (
    ...
    return;
  const remainingActiveLocks = allLockKeys.filter((k): boolean => {
    if (keysToClear.includes(k)) return false;
    const expiry = conn[k];
    return expiry && new Date(expiry).getTime() > now;
  });
  const clearObj: Record<string, unknown> = Object.fromEntries(
    keysToClear.map((k): [string, null] => [k, null]),
  );
  if (connLockExpired) {
