...
v0.0.79 -- active development

## Features

- **Multi-provider routing** -- OpenAI, Anthropic, Gemini, Codex, Ollama, 50+ providers
- **Compatibility APIs** -- OpenAI `/v1/*` endpoints with full format compliance (chat, responses, embeddings, audio, images, moderations, models, files)
- **Semantic cache** -- deduplicates identical requests; streaming responses cached too
...
- **PWA and offline-first** -- installable dashboard with service worker caching, offline reads, and mutation queue
...
| `REQUIRE_API_KEY`                 | `false`                                 | Require API keys on `/v1/*` routes and protected health/model endpoints |
...
Provider definitions live in `src/shared/constants/providers.ts`.

- **Free access**: Kiro AI, Qwen Code, Gemini CLI, iFlow AI, OpenCode Free
- **Free tier or API-key access**: OpenRouter, NVIDIA NIM, Ollama Cloud, Vertex AI, Gemini, Cloudflare, BytePlus ModelArk
- **OAuth and tool-account**: Claude Code, Antigravity, OpenAI Codex, GitHub Copilot, Cursor IDE, Kilo Code, Cline
- **API key and self-hosted**: GLM Coding, GLM (China), Kimi, Minimax Coding, Minimax (China), Alibaba, Alibaba Intl, Xiaomi MiMo, Volcengine Ark, OpenAI, Anthropic, OpenCode Go, Azure OpenAI, DeepSeek, Groq, xAI (Grok), Mistral, Together AI, Fireworks AI, Cerebras, Cohere, Nebius AI, SiliconFlow, Hyperbolic, Blackbox AI, Chutes AI, Ollama Local, Vertex Partner
