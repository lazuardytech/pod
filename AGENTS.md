...
- Zeabur PREBUILT services build from the Dockerfile **frozen in their service spec** (not the repo's current Dockerfile). After changing the repo Dockerfile, also update the service spec — MCP `deploy-from-specification` is silently broken; call GraphQL `api.zeabur.com/graphql` mutation `DeployFromSpecification(serviceID, specification{source{source:"GITHUB",repoID,branch,dockerfile},env[{key,default,expose}]})` directly with a Bearer `ZEABUR_TOKEN` (env round-tripped from `get-service-variables`).
- Spec-deploys (`deployFromSpecification`) **reset the service health check** back to the flapping default (`GET /`). Re-apply after any spec-deploy via GraphQL `updateServiceHealthCheckV2(serviceID, healthCheck{type: HTTP, port: "port-20140", http{path: "/api/health"}})`; a fresh `deploy(serviceID, environmentID, gitRef{repoID, ref})` is needed for the new probe to take effect (gotcha §35). `deployments(serviceID, environmentID)` exposes `commitSHA` to verify which commit a service actually runs.
- The GraphQL `deploy()` mutation **ignores `gitRef.ref` for PREBUILT_V2 services and deploys the repo default branch (`main`)** — verified 3× by `commitSHA` in `deployments()` (a `ref: "canary"` deploy landed at main's SHA). To ship a canary commit to pod-canary, **push to the `canary` branch** (the push-triggered deployment follows the pushed branch) — never `deploy()`. Verify every rollout by `commitSHA`, not by status alone. `cancelOngoingDeployment(deploymentID)` cancels a wrong-addressed rollout safely; the previous deployment keeps serving.
...
- `bun run format` / `check` / `lint` must use lockfile binaries (`oxfmt`, `oxlint`, `tsc`) — never `bun x oxlint`, which floats past bun.lock (1.79+ enables `react(set-state-in-effect)`). oxlint `--deny-warnings` treats warnings as errors (exit non-zero); without it, warnings print but check still passes. `bun run check` also runs `scripts/check-open-sse-ts-nocheck.ts` (any `@ts-nocheck` under `open-sse/` fails; the allowlist is empty since v0.0.87 — fix types, don't add entries) and `bun run check:cloud` (`cd cloud && bun install --frozen-lockfile && tsc --noEmit -p .`; cloud's usageDb stub is mirrored in `cloud/tsconfig.json` paths like the wrangler alias).
...
- MCP stack: Cloudflare + Context7 attach as native agent tools via the Hoplite dashboard (HTTP). Repo-declared stdio MCPs in `.hoplite/settings.json` (codegraph, pagespeed, zeabur, context7-fallback) are NOT surfaced as native tools — invoke via shell wrappers. Keyed wrappers read `ZEABUR_TOKEN` / `CONTEXT7_API_KEY` from the environment first, then gitignored `.env`.
