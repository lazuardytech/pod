...
function isStaticAssetRequest(request, url) {
  ...
  if (url.pathname.startsWith("/_next/static/"))
    return true;
  return request.destination === "script" || request.destination === "style" || request.destination === "font" || request.destination === "worker";
}
...
function hasSensitiveQuery(url) {
  for (const key of url.searchParams.keys()) {
    if (SENSITIVE_SEARCH_PARAMS.has(key.toLowerCase()))
      return true;
  }
  return false;
}
...
async function precacheShell() {
  ...
  const shellResults = await Promise.all(SHELL_ROUTES.map(async (route) => {
    try {
  ...
  const staticResults = await Promise.all(STATIC_PRECACHE.map(async (route) => {
    try {
      const response = await fetch(new Request(route, { cache: "reload" }));
      if (isCacheableResponse(response) && responseAllowsStorage(response)) {
  ...
  if (shellFailed.length > 0) {
    console.warn(`[Pod SW] precacheShell: ${shellFailed.length}/${SHELL_ROUTES.length} shell routes not cached`, shellFailed);
  }
  if (staticFailed.length > 0) {
    console.warn(`[Pod SW] precacheShell: ${staticFailed.length}/${STATIC_PRECACHE.length} static assets not cached`, staticFailed);
  }
...
async function handleImageRequest(request) {
  ...
  if (cached) {
    ...
    if (Date.now() - cacheTime < IMAGE_MAX_AGE_MS)
      return cached;
  }
  try {
    const response = await fetch(request);
...
function registerServiceWorker() {
  if (typeof sw === "undefined" || typeof sw.addEventListener !== "function")
    return;
  sw.addEventListener("install", (event) => {
    ...
    event.waitUntil((async () => {
      ...
      if (result.shellFailed.length > 0) {
        console.warn("[Pod SW] install precache incomplete — warmShellCache will retry on activate");
      }
    })());
  });
