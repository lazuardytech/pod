 ...
 */
export async function handleCleanup(env: Env): Promise<{ success: boolean; deleted?: number; cutoffDate?: string; error?: string }> {
  ...
  try {
    ...
    log.info("CLEANUP", `Deleted ${(result.meta as { changes?: number })?.changes || 0} old records`);
